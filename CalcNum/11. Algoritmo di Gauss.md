...
$$\sum_{h=1}^{n-1}{2(n-h)^2+(n-h)}$$

Riguardare

$$\sum_{h=1}^{n-1}{2(n-h)^2+(n-h)} \sim 2\frac nn$$
...
Applicando il metodo di Gauss si parte da Ax=b e si ottiene un sistema triangolare equivalente $Ux=\tilde{b}$ che si può risolvere con il metodo di [[10. Sistemi lineari#Algoritmo Di Sostituzione All'indietro|sostituzione all'indietro]]
